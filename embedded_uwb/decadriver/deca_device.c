 ...
 */
uint8_t dwt_otprevision(void) { return dw1000local.otprev; }
 ...
 */
uint32_t dwt_getpartid(void) { return dw1000local.partID; }
 ...
 */
uint32_t dwt_getlotid(void) { return dw1000local.lotID; }
 ...
 * no return value
 */
void dwt_readrxdata(uint8_t *buffer, uint16_t length, uint16_t rxBufferOffset) { dwt_readfromdevice(RX_BUFFER_ID, rxBufferOffset, length, buffer); }
...
void dwt_readdiagnostics(dwt_rxdiag_t *diagnostics) {
    ...
    dwt_readfromdevice(RX_FQUAL_ID, 0x0, 2, (uint8_t *)&diagnostics->stdNoise);
    dwt_readfromdevice(RX_FQUAL_ID, 0x2, 2, (uint8_t *)&diagnostics->firstPathAmp2);
    dwt_readfromdevice(RX_FQUAL_ID, 0x4, 2, (uint8_t *)&diagnostics->firstPathAmp3);
    dwt_readfromdevice(RX_FQUAL_ID, 0x6, 2, (uint8_t *)&diagnostics->maxGrowthCIR);
 ...
 */
uint32_t dwt_readtxtimestamphi32(void) { return dwt_read32bitoffsetreg(TX_TIME_ID, 1); }
 ...
 */
uint32_t dwt_readtxtimestamplo32(void) { return dwt_read32bitoffsetreg(TX_TIME_ID, 0); }
 ...
 */
uint32_t dwt_readrxtimestamphi32(void) { return dwt_read32bitoffsetreg(RX_TIME_ID, 1); }
 ...
 */
uint32_t dwt_readrxtimestamplo32(void) { return dwt_read32bitoffsetreg(RX_TIME_ID, 0); }
 ...
 */
uint32_t dwt_readsystimestamphi32(void) { return dwt_read32bitoffsetreg(SYS_TIME_ID, 1); }
 ...
 * no return value
 */
void dwt_readsystime(uint8_t *timestamp) { dwt_readfromdevice(SYS_TIME_ID, 0, SYS_TIME_LEN, timestamp); }
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
int dwt_readfromdevice(uint16_t recordNumber, uint16_t index, uint32_t length, uint8_t *buffer) {
    uint8_t header[3];  // Buffer to compose header in
    int cnt = 0;        // Counter for length of header
#ifdef DWT_API_ERROR_CHECK
    if (recordNumber > 0x3F) {
 ...
 * no return value
 */
void dwt_seteui(uint8_t *eui64) { dwt_writetodevice(EUI_64_ID, 0x0, 8, eui64); }
 ...
 * no return value
 */
void dwt_geteui(uint8_t *eui64) { dwt_readfromdevice(EUI_64_ID, 0x0, 8, eui64); }
            ...
            } else  // No overrun condition - proceed to process the frame
            {
                ...
                } else  // Double buffer
                {
                    ...
                    if (dwt_checkoverrun() == 0) {
                        ...
                        // The call-back has completed, but the overrun has been set, before we toggled, this means two new frames have arrived (one in the other buffer) and the 2nd's PHR good set the
                        // overrun flag Due to a receiver bug, which cannot guarantee the last frame's data was not corrupted need to reset receiver and discard any new data
                        dwt_forcetrxoff();
...
void dwt_setleds(uint8_t test) {
    ...
    if (test & 0x1) {
        ...
        buf[0] &= ~0xC0;
        buf[0] |= 0x40;
        dwt_writetodevice(GPIO_CTRL_ID, 0x00, 2, &buf[0]);  // TW
        // dwt_writetodevice(GPIO_CTRL_ID,0x01,1,&buf[1]);//ORIG
 ...
 * no return value
 */
void dwt_setdelayedtrxtime(uint32_t starttime) { dwt_write32bitoffsetreg(DX_TIME_ID, 1, starttime); }  // end dwt_setdelayedtrxtime()
...
int dwt_starttx(uint8_t mode) {
    ...
    if (mode & DWT_START_TX_DELAYED) {
        ...
        if ((checkTxOK & 0x0400) == 0)  // Transmit Delayed Send set over Half a Period away or Power Up error (there is enough time to send but not to power up individual blocks).
        {
            // printf("tx delayed \n");
 ...
 */
int dwt_checkoverrun(void) { return ((dwt_read16bitoffsetreg(SYS_STATUS_ID, 2) & (SYS_STATUS_RXOVRR >> 16)) == (SYS_STATUS_RXOVRR >> 16)); }
 ...
 * no return value
 */
void dwt_setpreambledetecttimeout(uint16_t timeout) { dwt_write16bitoffsetreg(DRX_CONF_ID, DRX_PRETOC_OFFSET, timeout); }
