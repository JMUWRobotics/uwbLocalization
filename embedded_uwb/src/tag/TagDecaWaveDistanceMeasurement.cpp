...
ROS_Topic<uwb::Distance> distanceT(-1, "/uwb/distance");
uwb::Distance distanceMsg;
ROS_Topic<std_msgs::UInt8> stateT(-1, "/uwb/state");
std_msgs::UInt8 stateMsg;
ROS_Topic<std_msgs::Bool> resetT(-1, "/uwb/reset");
std_msgs::Bool resetMsg;
static CommBuffer<std_msgs::Bool> resetBuffer;
static Subscriber resetSub(resetT, resetBuffer);
ROS_Topic<std_msgs::UInt8> activateT(-1, "/uwb/activate");
std_msgs::UInt8 activateMsg;
static CommBuffer<std_msgs::UInt8> activateBuffer;
static Subscriber activateSub(activateT, activateBuffer);
...
void DecaWaveDistanceMeasurement::init() {
    ...
    ledg.init(1, 1, 0);
    led.init(1, 1, 1);
    tag = new Node(getNodeNumber());
    uart_usb.init(115200);
    gw.init();
    gw.addPublisher(&distanceT);
    gw.addPublisher(&stateT);
    gw.addSubscriber(&resetT);
    gw.addSubscriber(&activateT);
}
...
void DecaWaveDistanceMeasurement::run() {
    init_decaWaveModule(&config, antenna_delays[tag->getId()]);
    ...
    while (1) {
        ...
        //check if a new msg was published on the activate topic:
        if(tag->getNodeIndex() == 0 && activateBuffer.getOnlyIfNewData(activateMsg)){
            numberOffActiveNodes = activateMsg.data;
            tag->toggleState();
            nextTime2Action = NOW();
        }

        //check if a new msg was published on the reset topic:
        if (resetBuffer.getOnlyIfNewData(resetMsg) && resetMsg.data){
            reset();
        }

        //publish the current state of the tag:
        stateMsg.data = (u_int8_t) tag->getState();
        stateT.publish(stateMsg);
        ...
        if (nextTime2Action <= NOW()) {
            switch (tag->getState()){
                ...
                case NodeState::ACTIVATE_NEXT_NODE:
                    ...
                    break;
                // if node 0 hasn't been activted from another node after 3 seconds, it sctivates itself:
                case NodeState::PAUSE:
                    if (tag->getNodeIndex() == 0){
                        tag->toggleState();
                        nextTime2Action = NOW();
                        break;
                    } else {
...
uint8_t DecaWaveDistanceMeasurement::next_destId(uint8_t offset/* = 0*/) {
    if (numberOffActiveNodes == 1){
        return tag->getNodeIndex();
    }
    return (tag->getNodeIndex() + 1 + offset) % numberOffActiveNodes;
}
...
void DecaWaveDistanceMeasurement::send_dist(float distance, uint8_t destId, uint8_t twrSourceId) {
    ...
    rx_buffer[DISTANCE_MSG_SOURCE_IDX] = twrSourceId;
    uwb_write(tag->getNodeIndex(), destId, tx_distance_msg, sizeof(tx_distance_msg));
}

void DecaWaveDistanceMeasurement::send_state(uint8_t destId){
    tx_state_msg[STATE_MSG_STATE_IDX] = (uint8_t) tag->getState();
    uwb_write(tag->getNodeIndex(), destId, tx_state_msg, sizeof(tx_state_msg));
}

void DecaWaveDistanceMeasurement::send_activate(uint8_t destId, uint8_t counter /* = 0*/){
    if (destId == tag->getNodeIndex()){
        tag->toggleState();
        return;
    }
    tx_activate_msg[ACTIVATE_MSG_NUMNODES_IDX] = numberOffActiveNodes;
    uwb_write(tag->getNodeIndex(), destId, tx_activate_msg, sizeof(tx_activate_msg));
...
void DecaWaveDistanceMeasurement::send_reset(){
    uwb_write(tag->getNodeIndex(), 255, tx_reset_msg, sizeof(tx_reset_msg));
}
...
void DecaWaveDistanceMeasurement::start_twr() {
    uwb_write(tag->getNodeIndex(), 255, tx_poll_msg, sizeof(tx_poll_msg));
...
void DecaWaveDistanceMeasurement::send_response(uint8_t destId) {
    uwb_write(tag->getNodeIndex(), destId, tx_resp_msg, sizeof(tx_resp_msg));
...
void DecaWaveDistanceMeasurement::send_final() {
    ...
    uint64_t final_tx_ts = final_tx_time + antenna_delays[tag->getId()];
    ...
    uwb_write(tag->getNodeIndex(), 255, tx_final_msg, 10 + (2 + numberOffActiveNodes) * FINAL_MSG_TS_LEN, DWT_START_TX_DELAYED);
}
...
float DecaWaveDistanceMeasurement::calculate_distance() {
    ...
    final_msg_get_ts(&rx_buffer[FINAL_MSG_RESP_RX_TS_IDX + tag->getNodeIndex() * FINAL_MSG_TS_LEN], &resp_rx_ts[tag->getNodeIndex()]);

    // check if the resp msg. didn't arrive:
    if (resp_rx_ts[tag->getNodeIndex()] == MAX_VALUE_64_BIT){
        return -1;
    }

    if (resp_rx_ts[tag->getNodeIndex()] < poll_tx_ts) {
        resp_rx_ts[tag->getNodeIndex()] += MAX_VALUE_40_BIT;
        final_tx_ts += MAX_VALUE_40_BIT;
    }
    if (final_tx_ts < resp_rx_ts[tag->getNodeIndex()]) {
        final_tx_ts += MAX_VALUE_40_BIT;
    }
    ...
    Ra = (double)(resp_rx_ts[tag->getNodeIndex()] - poll_tx_ts);
    Rb = (double)(final_rx_ts - resp_tx_ts);
    Da = (double)(final_tx_ts - resp_rx_ts[tag->getNodeIndex()]);
...
uint8_t DecaWaveDistanceMeasurement::receiveMessages() {
    if (uwb_read(rx_buffer)) {
        ...
        if (sourceId != tag->getNodeIndex()){ // only makes sence to read the msg if it isn't from itself
            switch (rx_buffer[ALL_MSG_TYPE_IDX]) {
                case MSG_TYPE_POLL: { // TWR poll message
                    poll_rx_ts = get_rx_timestamp_u64();
                    AT(NOW() + tag->getNodeIndex() * respWaitTimePerNode);
                    send_response(sourceId);
                    return MSG_TYPE_POLL;
                }
                case MSG_TYPE_RESP: { // TWR resp message
                    if (tag->getNodeIndex() == destId){
                ...
                case MSG_TYPE_FINAL: { // TWR final message
                    ...
                    if (distance > 0){
                        ...
                        distanceMsg.node1 = sourceId;
                        distanceMsg.node2 = tag->getNodeIndex();
                        distanceT.publish(distanceMsg);
                    }
                ...
                case MSG_TYPE_DISTANCE: { // distance message
                    if (tag->getNodeIndex() == destId){
                        ...
                        distanceMsg.distance = distance;
                        distanceMsg.node1 = rx_buffer[DISTANCE_MSG_SOURCE_IDX];
                        distanceMsg.node2 = sourceId;
                        distanceT.publish(distanceMsg);
