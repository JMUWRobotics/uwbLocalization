...
: public pointcloud
{
    ...
    importer() : pointcloud(false) // set this to true if you need gps aligned timing
  {
    ...
    roomMsg.header.frame_id="/riegl";
    roomMsg.angle_min = 0;
    roomMsg.angle_max = rad(360.0);
    roomMsg.angle_increment = rad(resp);
    roomMsg.time_increment  = 1/(freq*MAPBUCKETS);
    roomMsg.scan_time       = 1.0/freq;
    roomMsg.range_min = 0.05;
    roomMsg.range_max = 20.0;
    roomMsg.ranges.resize(MAPBUCKETS);
    roomMsg.intensities.resize(0);
    ...
    room_publisher = n.advertise<sensor_msgs::LaserScan>("room",100);
  ...
  protected:
    ...
    inline bool mapset(double x, double y, bool noroom) {
      ...
      if(tindex >= 0 && tindex < MAPBUCKETS) {
	    ...
	    roomMsg.ranges[tindex] = polar[0];
            if(roomMsg.ranges[tindex] < 0 || (roomMsg.ranges[tindex] > -1 && polar[1] < roomMsg.ranges[tindex])) {
              roomMsg.ranges[tindex] = polar[0];
            }
          }   
          delete[] polar;
    ...
    void sendMap() {    
      ...
      roomMsg.header.stamp = ros::Time::now();
      pointMsg.header.stamp = ros::Time::now();

      map_publisher.publish(laserMsg);
      room_publisher.publish(roomMsg);
      point_publisher.publish(pointMsg);
    }
    ...
    void on_echo_transformed(echo_type echo)
    {
      if (STATE != 1) {
        ...
        if ( point[2] > -0.30  && point[2] < 0.20) { // TODO FIXME
          if(range > 0.5) mapset(point[0], point[1], true); 
        } else if(point[2] > 1.60 && point [2] < 2.00) {
          mapset(point[0], point[1], false); 
          //delete point;
        }
        bool del = true;
        if(pointcount > 0) {
...
void prepareCloud() {
  ...
  ROS_ERROR("Delete %d %d", points.size(), pointcount);
  int i = 0;
